...
class LevelDBWriter(writableDB: DB, fs: FunctionalitySettings) extends Caches {
  ...
  override def addressTransactions(
    ...
    val filter = types.map(_.id)
      ...
      val txs = for {
        addressId <- db.get(k.addressId(address)).toSeq
  ...
  override def assetDistribution(height: Int, assetId: ByteStr) = readOnly { db =>
    ???
  }

  override def wavesDistribution(height: Int) = readOnly { db =>
    ???
  }
}
